...
namespace SchoolProjectAPI.Controllers
{
    ...
    public class PersonWidgetController : ControllerBase
    {
        ...
        [HttpGet]
        public ActionResult<List<LitePersonWidgetDTO>> Get()
        {
            return Ok(mapper.Map<IEnumerable<LitePersonWidgetDTO>>(repoWrapper.PersonWidget.Get()).ToList());
        }
        ...
        [HttpPost]
        public ActionResult Post([FromBody] LitePersonWidgetDTO value)
        {
        ...
        [HttpPut("{id}")]
        public ActionResult Put(long id, [FromBody] LitePersonWidgetDTO value)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
